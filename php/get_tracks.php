...
{
    protected $filename;
    public $title,$artist,$album,$year,$comment,$genre;

    public function __construct($filename)
    {
        $id_start=filesize($filename)-128;
        $fp=fopen($filename,"r");
        fseek($fp,$id_start);
        $tag=fread($fp,3);
    ...
    public function getMeta()
    {
        ...
        echo "Исполняет: {$this->artist}<br>";
        echo "Альбом: {$this->album}<br>";
        echo "Год: {$this->year}<br>";
        echo "Коммент: {$this->comment}<br>";
        echo "Жанр: {$this->genre}<br>";

        $duration1 = $this->getDurationEstimate();//(faster) for CBR only*/
        $duration2 = $this->getDuration();//(slower) for VBR (or CBR)
        /*echo "duration: $duration1 seconds"."\n";
        echo "estimate: $duration2 seconds"."\n";*/
        //echo "Длительность: ".self::formatTime($duration2)."\n";
        return self::formatTime($duration2);
    }
...
/*
$mp3file = new MP3File("../music/01 - God Bless The U.S.A. (Live 2014).mp3");//http://www.npr.org/rss/podcast.php?id=510282
$mp3file->getMeta();*/
$ret = array();
foreach (scandir('../music') as $file)
{
    ...
    {
        $mp3file = new MP3File("../music/$file");//http://www.npr.org/rss/podcast.php?id=510282
        $mus = array(
            'file' => $file,
            'duration'  => $mp3file->getMeta(),
        );
        $ret[$file] = $mus;
    }
